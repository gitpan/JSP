...
sub JSP::PerlHash::new { bless {}, 'JSP::PerlHash' }
sub JSP::PerlHash::get_ref { $_[0]; }
$JSP::PerlHash::construct_blessed = 1;

my $rt = JSP::Runtime->new();
my $cx = $rt->create_context();
$cx->bind_function(ok => \&ok);

ok($JSP::PerlHash::construct_blessed, "In legacy mode");
{
    my $hash = JSP::PerlHash->new();
    ok(defined $hash, "Defined");
    isa_ok($hash, "JSP::PerlHash");
...
/);
    ...
    isa_ok($hash, "JSP::PerlHash", "Legacy wrapper");
...
$JSP::PerlHash::construct_blessed = undef;
{
    my $hash = $cx->eval(q/
