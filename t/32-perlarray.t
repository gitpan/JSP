...
# Claes compatibility
sub JSP::PerlArray::new { bless [], 'JSP::PerlArray' }
sub JSP::PerlArray::get_ref { $_[0] }
$JSP::PerlArray::construct_blessed = 1; # Turn on legacy mode

my $cx = JSP->stock_context;
$cx->bind_all(ok => \&ok, is => \&is);

{
    my $arr = JSP::PerlArray->new();
    ok(defined $arr);
    isa_ok($arr, "JSP::PerlArray");
    ok($arr->get_ref);
    my $av = $arr->get_ref;
    is(reftype $av, "ARRAY");
...
$JSP::PerlArray::construct_blessed = undef;
