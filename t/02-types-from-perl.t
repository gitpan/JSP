...
$cx1->bind_all(
	...
	my $ref = shift;
	isa_ok($ref, 'HASH');
	isa_ok(JSP::Context::current->jsvisitor($ref), 'JSP::Visitor',
	    "Now a visitor");
    }, 
);
...

{
    my $h = {};
    my $h2 = { a => 1, 'b' => 2 };
    ok(!JSP::jsvisitor($h2), "Not a visitor yet");
    $cx1->call(test_hash => $h, $h2);
    ok(!JSP::jsvisitor($h2), "Was a visitor");
    $h = undef;
    $h2 = undef;
}
