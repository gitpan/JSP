...
sub bind_class {
    my $self = shift;
    my %args = @_;
    require JSP::PerlClass;
    my $jsclass = JSP::PerlClass->new(@_);
...
sub compile {
    ...
    JSP::Script->new($self, undef, $source, $name);
}

sub compile_file {
    local $_[0]->{RaiseExceptions} = 0;
    JSP::Script->new($_[0], undef, undef, $_[1]);
}
...
JSP::Context - An object in which we can execute javascript
...
Returns a newly born javascript object. This is equivalent to C<<
$ctx->eval('({})') >>
...
Bind a native class created L<with JSP::PerlClass>
...
Creates a new C<JSP::Context>-object in the supplied runtime.

=item create ( $runtime )

Creates a new context in JSP::Runtime $runtime returns it.
...
Returns the JSP::Runtime that created this context.
